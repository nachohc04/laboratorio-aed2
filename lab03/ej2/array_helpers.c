...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        // checkeo que la fecha este en el formato adecuado
        if (res != 3) {
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        if (!(k_year<=LST_YEAR && k_year>=FST_YEAR)) {
            fprintf(stderr, "Invalid date format year=%u : Year should be between %u and %u.\n", k_year, FST_YEAR, LST_YEAR);
            exit(EXIT_FAILURE);
        }
        if (!(k_month>=1 && k_month<=MONTHS)) {
            fprintf(stderr, "Invalid date format month=%u : Month should be between %u and %u.\n", k_month, january, december);
            exit(EXIT_FAILURE);
        }
        if (!(k_day>=FST_DAY && k_day<=LST_DAY)) {
            fprintf(stderr, "Invalid date format day=%u : Day should be between %u and %u.\n", k_day, FST_DAY, LST_DAY);
            exit(EXIT_FAILURE);
        }
